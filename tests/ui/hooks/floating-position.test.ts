...
describe("computeFloatingRect", () => {
  ...
  it("keeps a document-coord popover attached to a visible trigger", () => {
    const result = computeFloatingRect(
      rectAt(200, 40),
      DOCUMENT_PLACEMENT,
    ...
    // Natural position wins: rect.bottom (240) + gap (4) + scrollY (400) = 644
    expect(result.top).toBe(644);
  });

  it("never clamps a document-coord popover, even when the trigger sits above the visible region", () => {
    const result = computeFloatingRect(
      rectAt(100, 40),
      DOCUMENT_PLACEMENT,
  ...
  it("stays below when there isn't enough room on either side", () => {
    const result = computeFloatingRect(
      rectAt(80, 32),
      VIEWPORT_PLACEMENT,
      { offsetTop: 0, height: 200 },
  ...
  it("clamps arrowLeft into the panel when the panel got shoved sideways", () => {
    ...
    // Panel pushed left to fit within viewport (390 - 8 - 280 = 102)
    expect(result.left).toBe(102);
    // Trigger centre = 385, past the panel width — clamp to width - 14 = 266
