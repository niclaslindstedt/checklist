...
//! The Checklist desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Checklist does — the
...
use checklist_shell::config::APP_SCHEME;
use checklist_shell::oauth::{AWAIT_PATH, BEGIN_PATH};
use checklist_shell::webroot::webroot_exists;
...
fn main() {
    tauri::Builder::default()
        ...
        // `checklist_shell::webroot` for the three properties this arrangement is
        ...
        // `checklist_shell::oauth`), which can be minutes, so it answers from a
        ...
        .setup(|app| {
            ...
            if checklist_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Checklist could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Checklist desktop shell could not start");
}
