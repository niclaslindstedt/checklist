...
# Checklist — the desktop app

A desktop wrapper around Checklist for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Checklist does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `checklist://` off the bundled `webroot/`               |
...
`cargo test -p checklist-shell` runs the entire decision layer on a machine with a
...
**The origin is the one thing to be careful with.** The user's checklists live in
...
`file://` page is an opaque origin, which would leave the checklists unable to
survive a launch.
...
| Variable        | Effect                                                               |
| --------------- | -------------------------------------------------------------------- |
| `CHECKLIST_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `CHECKLIST_WEBROOT` | Serve the site from somewhere else without rebuilding                |
...
identifier (`dev.local.checklist`). What an installed copy is called and the
