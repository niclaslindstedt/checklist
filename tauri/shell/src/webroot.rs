...
//! wrong to simplify.
//!
//! The site is served from a registered private scheme (`checklist://`, see
...
//! to lose:
//!
//!  1. **One stable origin.** The user's checklists live in IndexedDB and their
//!     settings in `localStorage`, both keyed by origin. `checklist://` is a
//!     constant, so the checklists survive every update. A `file://` page (or a
//!     server on an ephemeral port) would hand the user a different origin —
//!     and an empty checklists — at some point.
//!
...
/// `checklist:///C:/…` must not become a drive letter.
