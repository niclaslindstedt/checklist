...
//! shipping: `CHECKLIST_APP_URL` points the window at a remote URL instead (the
//! `/preview/` deploy slot, say), and `CHECKLIST_WEBROOT` serves a different
...
/// IndexedDB unable to keep a checklist between launches. A registered scheme
/// gives one stable origin that the checklists are keyed to for the life of the
/// install.
pub const APP_SCHEME: &str = "checklist";
...
/// CONSTANT — the origin is what the user's checklists are keyed to, so changing
...
/// `#0f1115`), painted behind the page so no white flash shows through while
/// it loads.
pub const BRAND_BG: (u8, u8, u8, u8) = (0x0f, 0x11, 0x15, 0xff);

/// What the window is called.
pub const WINDOW_TITLE: &str = "Checklist";

/// The window's opening size, and the smallest it may be dragged to.
///
/// The floor is the narrowest a list is still usable at — below it the app
/// is not broken, it is just a smaller phone.
