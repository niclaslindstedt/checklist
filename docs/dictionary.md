...
| **Checked items aren't editable** / **done item is locked** / **checked but not crossed** / **uncheck it to edit** | `enterEdit` / `onTitleTap` / `titleInert` (`src/ui/ChecklistRow.tsx`); the `item.checked` commit-and-close effect in `src/ui/ChecklistRowEditor.tsx`. [→](overview.md#edit-item) |
| **Stranded editor** / **row stuck open in edit mode** / **still active after I pressed another item** | the outside-`pointerdown` backstop (guarded on `document.activeElement`) in `src/ui/ChecklistRowEditor.tsx`. [→](overview.md#edit-item) |
...
| **Link in a note** / **clickable link** / **tappable link** / **autolink** / **open the link instead of editing** | `AUTOLINK`, `autolinkHref`, `anchor`, `insideLink` (`src/ui/markdown/renderMarkdown.tsx`); the `closest("a")` click guard + `e.target !== e.currentTarget` key guard on the rendered body in `src/ui/ChecklistRow.tsx`. [→](overview.md#link-in-a-note) |
...
| **Typeahead** / **type ahead** / **autocomplete** / **suggestions** / **suggest from the archive** | `suggestionPool` on `src/ui/AddItemForm.tsx`; `archivedTitlePool`, `suggestTitles` (`src/domain/suggestions.ts`); `matchPlainText`, `segmentMatches` (`src/domain/search.ts`). [→](overview.md#archive-typeahead) |
| **Archive view** / **the archive**                     | `src/ui/ArchiveView.tsx`. [→](overview.md#archive-view)                                                  |
| **Archived drawer** / **archive drawer** / **swipe up to reveal the archive** / **the bottom drawer** | `src/ui/ArchivedDrawer.tsx`; the swipe-up gesture `useSwipeUpReveal` (`src/ui/hooks/useSwipeUpReveal.ts`); wired in `src/ui/ChecklistView.tsx` (`archivedForActive`, `archiveDrawerOpen`). [→](overview.md#archived-drawer) |
...
| **Swipe to remove** (sidebar) / **swipe a list/namespace away** / **left swipe reveal** | `useSwipeReveal` (`src/ui/hooks/useSwipeReveal.ts`); the `SwipeToRemove` wrapper and `ChecklistRowStrip` in `src/ui/SideMenuRows.tsx`. [→](overview.md#swipe-to-remove-sidebar) |
| **Right swipe reveal** (sidebar) / **configure strip** / **the clock and template buttons** / **swipe a list right** | The leading strip of `ChecklistRowStrip` (`src/ui/SideMenuRows.tsx`) — `onSchedule` (`ClockIcon`) and `onSaveAsTemplate` (`TemplateIcon`); `leadingWidth` / `side` on `useSwipeReveal` (`src/ui/hooks/useSwipeReveal.ts`); wired in `renderChecklistRow` (`src/ui/SideMenu.tsx`). [→](overview.md#swipe-to-remove-sidebar) |
...
| **Browser back** / **back button** / **forward button** / **go back to the previous list** / **router navigation** / **history entries** | `useNavHistory`, `NavDestination`, `markNavigation`, `readNavDestination` (`src/app/use-nav-history.ts`); the verb wrapping + `applyDestination` in `src/app/use-app-navigation.ts`. [→](overview.md#browser-back--forward) |
| **Link to a list** / **bookmark a list** / **deep link** / **the URL of a checklist** / **`#list=`** | `destinationFragment`, `parseDestinationFragment`, `resolveUrlDestination`, `destinationUrl` (`src/app/nav-url.ts`); read on cold start by `useNavHistory` (`src/app/use-nav-history.ts`). [→](overview.md#bookmarking-a-list) |
...
| **Footer collapse rail** / **collapse the footer** / **fold the footer** / **collapse menu footer button** | `FooterCollapseRail` (`src/ui/SideMenuRows.tsx`); device-local `useFooterCollapsed` + `FOOTER_COLLAPSED_KEY` (`src/ui/hooks/useFooterCollapsed.ts`); wired in `src/ui/SideMenu.tsx`; `menu.collapseFooter` / `menu.expandFooter` strings. [→](overview.md#side-menu) |
| **Header menu** / **burger menu**                      | `src/ui/HeaderMenu.tsx`. [→](overview.md#header-menu)                                                    |
| **Checklist title** / **the list name** / **rename the list** / **press the title to rename** / **the keyboard doesn't come up for the title** / **can't get out of the title field** | `src/ui/ChecklistTitle.tsx` (`editing`, `autoEdit`, the `flushSync` opener plus the focus and outside-`pointerdown` layout effects); keyed by the open entry in `src/ui/ChecklistView.tsx`. [→](overview.md#checklist-title) |
| **Name a new list** / **name it right away** / **the new list opens its name field** / **rename bus** | `RenameChecklistContext`, `useRenameChecklist` (`src/ui/rename-checklist.ts`); `createList` (`src/ui/SideMenu.tsx`); the pending id + provider in `src/app/App.tsx`; drained in `src/ui/ChecklistView.tsx`; the ids returned by `addChecklist` / `addChecklistInFolder` (`src/app/use-checklist-lists.ts`). [→](overview.md#name-a-new-list) |
| **Checklist glyph** / **list icon** / **list mark** / **pick a list icon/colour** / **the icon by the title** | `src/ui/ChecklistGlyphButton.tsx`; `glyph` / `color` on `Checklist` + `setChecklistAppearance` (`src/domain/checklists.ts`, `src/app/use-checklist-lists.ts`); `CHECKLIST_GLYPH_NAMES`, `DEFAULT_CHECKLIST_GLYPH` (`src/ui/glyphs.ts`); also drawn on the list's sidebar row (`ChecklistSummary.glyph`/`color` → `renderChecklistRow` in `src/ui/SideMenu.tsx`); reuses `GlyphGrid` / `ColorPalette` / `NamespaceGlyph` / `FloatingPanel`. [→](overview.md#checklist-glyph) |
...
| **Copy button** / **copy glyph** / **copy/paste glyph** / **copy the list** | `src/ui/CopyButton.tsx`; `checklistBodyMarkdown` + `ChecklistBodyOptions` (`src/storage/markdown/codec.ts`); `CopyIcon` (`src/ui/icons.tsx`). [→](overview.md#copy-checklist) |
| **Copy scope menu** / **copy one category** / **copy just this section** / **copy All** | The `FloatingPanel` menu in `src/ui/CopyButton.tsx` (`app.copyAll` / `app.copyScope`); `activeCategories` (`src/domain/item-ops.ts`); `categoryId` / `categoryMarkers` on `ChecklistBodyOptions` (`src/storage/markdown/codec.ts`). [→](overview.md#copy-scope-menu) |
...
| **Date picker** / **calendar** / **due-date field** / **the date field** / **month picker** / **year picker** | `DatePicker` (`src/ui/form/DatePicker.tsx`); pure grid maths `buildMonthGrid` / `yearRangeStart` / `addMonths` / `parseISODate` / `toISODate` (`src/domain/calendar.ts`); `FloatingPanel` (`src/ui/FloatingPanel.tsx`). Custom calendar replacing the native `<input type="date">`, which broke month navigation in the iOS PWA; the caption drills day → month → year for quick jumps. Its `min` prop greys out every day / month / year before a floor day — the due-date field passes the chosen “not before” day. [→](overview.md#date-picker) |
| **Right-click menu** / **context menu** / **desktop actions menu** / **long-press menu** / **long press a row** | `ContextMenu` (`src/ui/ContextMenu.tsx`); `useContextMenu` (`src/ui/hooks/useContextMenu.ts`); `useDesktopPointer` (`src/ui/hooks/useMediaQuery.ts`); the touch long-press opener `useLongPress` (`src/ui/hooks/useLongPress.ts`). [→](overview.md#right-click-menu) |
| **Hold to select** / **text selection** / **the long-press selects the whole page** / **select mode** | The root `user-select` suppression in `src/styles/theme.css` (`@media (hover: none) and (pointer: coarse)`, plus the `#app` rule and the `input` / `textarea` opt-back-in); `clearNativeSelection` (`src/ui/clear-selection.ts`), called by `useLongPress` (`src/ui/hooks/useLongPress.ts`) and `AddItemButton` (`src/ui/AddItemButton.tsx`). [→](overview.md#hold-to-select) |
...
| **Timing** / **timing modal** / **deadline** / **due date** / **date row** / **set a deadline** / **clock button** / **recurring** / **recurrence** / **repeat every n weeks** | `deadline` / `recurrence` / `Recurrence` / `TimingPatch` on `ChecklistItem` (`src/domain/types.ts`); `setItemTiming` (`src/domain/item-ops.ts`), `deadlineStatus` / `daysUntil` / `addRecurrence` / `nextOccurrence` (`src/domain/deadlines.ts`); the dated-float `floatDatedToBottom` / `displayItems` (`src/domain/item-display.ts`); `setTiming` verb (`src/app/use-checklist-edits.ts`); `TimingModal` (`src/ui/TimingModal.tsx`), `TimingRow` (`src/ui/TimingRow.tsx`), the clock button + date row in `src/ui/ChecklistRow.tsx`; `ClockIcon` / `RepeatIcon` (`src/ui/icons/action.tsx`); the `*(due …)*` markdown marker in `src/storage/markdown/codec.ts`. [→](overview.md#timing-deadlines-and-not-before) |
| **Refresh** / **repeat without a due date** / **repeat as a refresh** / **bring it back** / **comes back unchecked** / **buy milk every week** / **daily repeat** / **repeat at a time of day** | `refreshAt` on `ChecklistItem` + `Recurrence.at` (`src/domain/types.ts`); `isRefreshing` / `nextRefreshAt` / `dueRefreshes` / `refreshItems` / `applyRefreshes` / `DEFAULT_REFRESH_TIME` (`src/domain/item-refresh.ts`); the stamp in `toggleItem` / `setAllChecked` and the clear in `setItemTiming` (`src/domain/item-ops.ts`); the pass in `src/app/use-scheduled-resets.ts`; the repeat + time fields in `src/ui/TimingModal.tsx`, the standalone repeat half of `src/ui/TimingRow.tsx`; the `*(every … , back …)*` markdown marker in `src/storage/markdown/codec.ts`. [→](overview.md#refresh-a-repeat-with-no-due-date) |
| **Not before** / **held back** / **gated item** / **not open yet** / **disabled checkbox** | `notBefore` on `ChecklistItem` (`src/domain/types.ts`); `isHeldBack` (`src/domain/deadlines.ts`) — the single test behind the hold; enforced in `toggleItem` / `setAllChecked` (`src/domain/item-ops.ts`); the “Not before” field in `TimingModal` (`src/ui/TimingModal.tsx`), the colourless label in `TimingRow` (`src/ui/TimingRow.tsx`), the `disabled` checkbox in `src/ui/ChecklistRow.tsx` and `src/ui/ChecklistRowEditor.tsx`; the `*(not before …)*` markdown marker in `src/storage/markdown/codec.ts`. [→](overview.md#timing-deadlines-and-not-before) |
| **Sort due dates to the top** / **sort held-back items to the bottom** / **dated first** / **held last** / **date sorting** | `sortDatedToTop` / `sortHeldToBottom` settings (`src/settings/types.ts`, defaults in `src/settings/store.ts`), toggles in `src/ui/settings/tabs/lists.tsx`; `floatDatedToTop` / `sinkHeldToBottom` / `displayItems` / `DisplayOrder` (`src/domain/item-display.ts`); the `displayOrder` value assembled in `src/app/App.tsx`. Both on by default; pure view sorts. [→](overview.md#timing-deadlines-and-not-before) |
| **Gate grouping** / **one date for a run** / **repeated not-before date** | `DisplayRow.sameGateAsPrevious`, set by `flattenForDisplay` (`src/domain/item-display.ts`); passed down as the `sameGate` prop through `src/ui/ChecklistView.tsx` → `src/ui/ChecklistRow.tsx` → `src/ui/TimingRow.tsx`. [→](overview.md#timing-deadlines-and-not-before) |
| **Timing glyphs** / **the two clocks** / **bar-and-clock** | `NotBeforeIcon` (`\|—◷`) and `DeadlineIcon` (`◷—\|`) in `src/ui/icons/action.tsx`, rendered by `TimingRow` (`src/ui/TimingRow.tsx`). A mirrored pair — the bar sits on the side the bound is. [→](overview.md#timing-deadlines-and-not-before) |
| **Reset schedule** / **scheduled reset** / **reset a list every day** / **refresh every** / **the schedule modal** / **clock glyph button** (sidebar) / **before-leaving-home list** | `ResetSchedule` / `ResetScheduleUnit` / `ResetSchedulePatch`, `resetSchedule` + `lastResetAt` on `Checklist` (`src/domain/types.ts`); the maths + verbs `nextResetAt` / `dueResetAt` / `dueResets` / `applyResets` / `resetChecklist` / `setResetSchedule` (`src/domain/reset-schedule.ts`); `setChecklistResetSchedule` (`src/app/use-checklist-lists.ts`); the pass `useScheduledResets` (`src/app/use-scheduled-resets.ts`); `ResetScheduleModal` (`src/ui/ResetScheduleModal.tsx`), host `src/app/modals/ResetScheduleModalHost.tsx`, `{ kind: "reset-schedule", checklistId }` on the modal bus; the `reset:` / `reset-since:` / `reset-last:` frontmatter in `src/storage/markdown/codec.ts`. [→](overview.md#reset-schedule) |
| **Reset pop-up** / **fresh start** / **pop up after refresh** / **the list that hovers over the active one** | `popUp` on `ResetSchedule` (`src/domain/types.ts`); `resetPopupListId` / `dismissResetPopup` (`src/app/use-scheduled-resets.ts`, on the checklist context); `ResetPopupModal` (`src/ui/ResetPopupModal.tsx`), mounted in `src/app/App.tsx`. [→](overview.md#reset-pop-up) |
...
| **Category** / **promote to category** / **category header** / **make it a category** / **store name** / **section header** | `ChecklistItem.category` (`src/domain/types.ts`); `setCategory` (`src/domain/item-ops.ts`, `src/app/use-checklist-edits.ts`); the `*(category)*` marker in `src/storage/markdown/codec.ts`; `activeCategories` (`src/domain/item-ops.ts`); the menu entry in `src/ui/ChecklistView.tsx` (`rowMenuItems`); category styling in `src/ui/ChecklistRow.tsx`. [→](overview.md#categories) |
| **Category add button** / **the + on a category** / **plus on the category row** / **add to this category** | The `PlusIcon` button rendered left of the grip on a category row in `src/ui/ChecklistRow.tsx` (`category && onAddChild`); `app.addToCategory` string; opens `composer.startChild` (`src/ui/hooks/useComposer.ts`) via `onAddChild` in `src/ui/ChecklistView.tsx`. [→](overview.md#categories) |
| **Empty the archive** / **empty archive** / **clear the archive** / **wipe the archive** | `emptyArchive` (`src/domain/archive-ops.ts`, via `src/app/use-checklist-lists.ts`); the trash button + confirm in the header of `src/ui/ArchiveView.tsx`. [→](overview.md#empty-the-archive) |
...
| **Drop at the bottom** / **drag to the very bottom** / **drop below the list** / **make it a root item again** / **the empty space under the list** | `resolveDropTarget` + `Rect.depth` (`src/ui/hooks/useListReorder.ts`); the `data-reorder-depth` attribute on the row `<li>` in `src/ui/ChecklistRow.tsx`. [→](overview.md#sub-items--nested-items) |
...
| **Save as template** / **extract a template** / **capture a list** / **create template** | `extractTemplate` (`src/domain/templates.ts`); `saveChecklistAsTemplate` (`src/app/use-checklist-lists.ts`); the row menu entry in `src/ui/SideMenu.tsx` + the right-swipe strip action in `ChecklistRowStrip` (`src/ui/SideMenuRows.tsx`). [→](overview.md#save-as-template) |
...
| **Item count** / **progress count** / **checked/total** / **show item count** / **count dropdown** / **check all** / **uncheck all** | `ItemCount` (`src/ui/ItemCount.tsx`); `showItemCount` (`src/settings/types.ts`); Lists-tab toggle (`src/ui/settings/tabs/lists.tsx`); `showItemCount` on `ChecklistContextValue` (`src/ui/checklist-context.ts`); bulk verbs `checkAll` / `uncheckAll` (`src/app/use-checklist-edits.ts`), `setAllChecked` (`src/domain/checklists.ts`). [→](overview.md#show-item-count) |
| **Count categories** / **don't count categories** / **categories in the count** / **the count includes the headers** / **it never reaches n/n** | `countCategories` (`src/settings/types.ts`); Lists-tab toggle (`src/ui/settings/tabs/lists.tsx`); `countableItems` (`src/domain/item-display.ts`, re-exported from `src/domain/checklists.ts`); `checkedCount` / `visibleCount` in `src/app/use-checklist.ts`; the `countCategories` arg on `progress` feeding `ChecklistSummary.remaining` (`src/app/use-checklist-lists.ts`). [→](overview.md#count-categories) |
...
| **Transform tab** / **transforms** / **transform rules** / **rewrite how items look** / **regex rules** | `src/ui/settings/tabs/transform.tsx`; `Settings.transforms` (`src/settings/types.ts`); engine `applyTransforms`, `activeTransforms`, `compilePattern`, `TransformRule`, `TransformSegment` (`src/domain/transforms.ts`); rendering `renderTransformed` / `renderSegment` (`src/ui/markdown/renderTransformed.tsx`); `transforms` on `MarkdownOptions` (`src/ui/markdown/renderMarkdown.tsx`) and on `ChecklistContextValue` (`src/ui/checklist-context.ts`). [→](overview.md#transforms) |
| **Rules for work vs home** / **transforms per namespace** / **namespace-scoped rules** / **Rules for picker** | `Settings.transforms` is a `TransformRules` map keyed by namespace slug; `namespaceTransforms`, `setNamespaceTransforms`, `hasAnyTransforms` (`src/domain/transforms.ts`); the picker + `namespaces` / `activeNamespace` props on `TransformTab` (`src/ui/settings/tabs/transform.tsx`); `settings.transform.namespace*` strings. [→](overview.md#transforms) |
| **Transform rule editor** / **add a transform** / **the regex modal** / **sample and result** | `TransformRuleModal` (`src/ui/settings/TransformRuleModal.tsx`). [→](overview.md#transform-rule-editor) |
| **Regex helper** / **insert dropdown** / **regex cheat sheet** / **what does `\d` mean** | `RegexHelper`, `TOKENS` (`src/ui/settings/RegexHelper.tsx`); `settings.transform.token.*` strings. [→](overview.md#transform-rule-editor) |
| **Turn `#134` into a link** / **link transform** / **issue link** | `kind: "link"` + `safeTransformHref`, `expandTemplate` (`src/domain/transforms.ts`). [→](overview.md#transforms) |
| **Mask sensitive text** / **hide a phone number** / **`076****123`** / **sensitive transform** | `kind: "sensitive"` + `maskValue`, `MaskStyle` (`src/domain/transforms.ts`). [→](overview.md#transforms) |
...
| **Namespace favicon** / **browser-tab icon** / **re-badge the tab** | `namespaceFaviconSrc`, `applyFaviconHref` (`src/ui/namespace-favicon.ts`), applied via an effect in `src/app/App.tsx`. [→](overview.md#namespace-glyph-and-favicon) |
...
| **Dropbox backend** / **Dropbox app folder** / **`free-checklist`** | `src/storage/dropbox/index.ts` (`DROPBOX_APP_FOLDER` — the `Apps/` folder name, from `VITE_DROPBOX_APP_FOLDER`; `dropboxWebUrl`). [→](overview.md#dropbox-backend) |
| **Google Drive backend** / **gdrive**                | `src/storage/gdrive/index.ts`. [→](overview.md#google-drive-backend)              |
| **iCloud backend** / **iCloud sync**                 | `ICloudStorageAdapter` (`src/storage/icloud/index.ts`); account-less, iOS-wrapper-only. [→](overview.md#native-wrapper) |
| **Native bridge** / **`window.__native`** / **postMessage bridge** | `src/storage/native-bridge.ts` (`isICloudAvailable` / `isWidgetsAvailable` / `isNotificationsAvailable`, web-side feature detection); `native/src/nativeBridge.ts` (the injected shim + message handler); `native/src/icloud.ts` (`NSUbiquitousKeyValueStore` via `react-native-icloudstore`); `native/src/widgets.ts` (widget host); `native/src/notifications.ts` (notification host). [→](overview.md#native-wrapper) |
| **Widgets** / **Home Screen widget** / **Lock Screen widget** / **the progress ring** / **due today** / **quick add** / **check-off widget** | `buildWidgetSnapshot` (`src/domain/widget-snapshot.ts`); `useWidgetMirror` (`src/app/use-widget-mirror.ts`) + `src/storage/native-widgets.ts` (publish / drain); `useWidgetDeepLink` (`src/app/use-widget-deep-link.ts`); native: `native/modules/widget-bridge` (Expo module), `native/targets/widget/*.swift` (WidgetKit), `native/widgets/android/*.kt` (Glance), `native/plugins/withWidgets.js`. Read-optimised mirror; interactive check-off writes back through the normal edit path. [→](overview.md#home-screen-widgets) |
| **Deadline reminders** / **notifications** (native) / **remind me** / **lead time** / **day before** / **week before** | `buildNotificationSchedule` / `documentHasDeadline` (`src/domain/notification-schedule.ts`); `useNotificationScheduler` (`src/app/use-notification-scheduler.ts`) + `src/storage/native-notifications.ts` (publish / permission); `deadlineReminders` + `reminderLeadDays` settings (`src/settings/types.ts`), toggles in `src/ui/settings/tabs/general.tsx`; native host `native/src/notifications.ts` (`expo-notifications`). Derived, re-armed-from-scratch mirror; native-only, opt-out. [→](overview.md#deadline-reminders) |
| **Native app** / **the iOS app** / **the wrapper** | `native/src/App.tsx` (the WebView); `native/src/useStaticServer.ts` (loopback server, pinned port); `native/src/webroot.ts`; `native/plugins/withWebroot.js` (bundle → native projects); `IS_NATIVE` (`src/build-env.ts`). Thin wrapper around the embedded web build — no separate native UI. iCloud sync is wired through the native bridge (iOS only). [→](overview.md#native-wrapper) |
...
| **Check all / uncheck all**       | press the header count → dropdown → `checkAll` / `uncheckAll` → `setAllChecked`. [→](overview.md#show-item-count) |
| **Edit an item / its body**       | press the row → `ChecklistRowEditor` → `editItem`. [→](overview.md#edit-item)      |
| **Delete an item**                | swipe-left → trash glyph (or right-click → menu, desktop) → `deleteItem`. [→](overview.md#delete-an-item) |
| **Set an item’s timing (not before / due date / recurring)** | swipe-left → clock glyph (or right-click → menu, desktop) → `TimingModal` → `setTiming`. [→](overview.md#timing-deadlines-and-not-before) |
...
| **Save a list as a template**     | side-menu right-click → Save as template (desktop), swipe-right → template button (touch) → `saveChecklistAsTemplate`. [→](overview.md#save-a-list-as-a-template) |
| **Put a list on a reset schedule** / **make a list reset itself** | side-menu right-click → Reset schedule (desktop), swipe-right → clock button (touch) → `ResetScheduleModal` → `setChecklistResetSchedule`. [→](overview.md#put-a-list-on-a-reset-schedule) |
