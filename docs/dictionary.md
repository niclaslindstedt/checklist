...
| **Keyboard nav bar** / **edit nav bar** / **up/down buttons above the keyboard** / **navigate items while editing** / **double / duplicate keyboard bar** | `src/ui/EditNavBar.tsx`; `ActiveEditor` (`src/ui/edit-nav.ts`); `activeEditor` + `moveEdit` (`src/ui/ChecklistView.tsx`); `onActiveChange` (`src/ui/ChecklistRowEditor.tsx`); native-bar suppression via `ContentEditable` (`src/ui/form/ContentEditable.tsx`). [→](overview.md#keyboard-nav-bar) |
