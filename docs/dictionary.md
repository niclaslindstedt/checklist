...
| **Deadline** / **due date** / **date row** / **set a deadline** / **clock button** / **recurring** / **recurrence** / **repeat every n weeks** | `deadline` / `recurrence` / `Recurrence` on `ChecklistItem` (`src/domain/types.ts`); `setItemDeadline` (`src/domain/item-ops.ts`), `deadlineStatus` / `daysUntil` / `addRecurrence` / `nextOccurrence` (`src/domain/deadlines.ts`); the dated-float `floatDatedToBottom` / `displayItems` (`src/domain/item-display.ts`); `setDeadline` verb (`src/app/use-checklist-edits.ts`); `DeadlineModal` (`src/ui/DeadlineModal.tsx`), `DeadlineRow` (`src/ui/DeadlineRow.tsx`), the clock button + date row in `src/ui/ChecklistRow.tsx`; `ClockIcon` / `RepeatIcon` (`src/ui/icons/action.tsx`); the `*(due …)*` markdown marker in `src/storage/markdown/codec.ts`. [→](overview.md#deadlines) |
...
| **Set a deadline / make it recurring** | swipe-left → clock glyph (or right-click → menu, desktop) → `DeadlineModal` → `setDeadline`. [→](overview.md#deadlines) |
