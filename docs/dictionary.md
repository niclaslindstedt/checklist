...
| **Native bridge** / **`window.__native`** / **postMessage bridge** | `src/storage/native-bridge.ts` (`isICloudAvailable`, web-side feature detection); `native/src/nativeBridge.ts` (the injected shim + message handler); `native/src/icloud.ts` (`NSUbiquitousKeyValueStore` via `react-native-icloudstore`). [→](overview.md#native-wrapper) |
