...
| **Date picker** / **calendar** / **due-date field** / **the date field** | `DatePicker` (`src/ui/form/DatePicker.tsx`); pure grid maths `buildMonthGrid` / `addMonths` / `parseISODate` / `toISODate` (`src/domain/calendar.ts`); `FloatingPanel` (`src/ui/FloatingPanel.tsx`). Custom calendar replacing the native `<input type="date">`, which broke month navigation in the iOS PWA. [→](overview.md#date-picker) |
