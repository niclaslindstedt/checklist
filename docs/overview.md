...
(`native/app.json`). The same bridge is the channel the Home Screen widgets
build on.

### Home Screen widgets

The native app ships Home Screen / Lock Screen widgets — a WidgetKit extension
on iOS, Glance on Android. Widgets render in a **separate OS process** that
can't reach the WebView's `localStorage`, so the app can't hand them the live
document. Instead it mirrors a compact, derived **snapshot** out to a shared
container the widget process can read (an App Group on iOS, a shared
`SharedPreferences` file on Android), and the widget draws from that.

- **The snapshot** — `buildWidgetSnapshot` (`src/domain/widget-snapshot.ts`) is
  a pure projection of the document: the active list's progress and next open
  items, a per-list summary for the configurable list picker, and the
  due-today / overdue items across every list (recurring deadlines resolved to
  their next real occurrence). It is **read-optimised and derived** — the
  WebView storage stays the source of truth.
- **Publishing it** — `useWidgetMirror` (`src/app/use-widget-mirror.ts`)
  rebuilds the snapshot whenever the document or active list changes and pushes
  it (debounced) through `window.__native.widgets.publish`
  (`src/storage/native-widgets.ts`). The native side
  (`native/modules/widget-bridge`, a local Expo module) writes it to the shared
  container and calls `WidgetCenter.reloadAllTimelines()` / nudges Glance.
- **Write-back (interactive check-off)** — the one widget that mutates state
  can't write the store from its own process either, so its tap queues a
  `{ type: "toggle", listId, itemId }` action in the shared container. The app
  drains that queue on foreground (and on a native signal) and replays each
  through the **normal edit path** (`toggleItemInList` →
...
The iOS extension target is generated by `@bacons/apple-targets` from
`native/targets/widget/` (SwiftUI + App Intents); the Android Glance provider
lives in `native/widgets/android/` and is wired into the app project by
`native/plugins/withWidgets.js`. Both sides address one **App Group**,
`group.se.niclaslindstedt.checklist`, declared in `native/app.json` and the
