...
This same per-list markdown layout is what [cloud sync](feature:cloud-sync) uses too: **Dropbox** and **Dropbox** store one markdown file per list, exactly like a local folder. Only the default **This device** option keeps everything in a single bundled document instead of separate files.
