...
clock. Tap it to open the deadline picker: choose a due date, and if the
task comes back around, how often it repeats — every so many weeks,
...
A dated item wears a slim date row just above its title, in a smaller
font so it reads as a caption rather than a second line of content. The
row shows the due date (and a repeat summary like "every 2 weeks" when
the item recurs), and it changes colour as the day gets closer:
...
Dated items gather at the bottom of the unchecked part of the list,
sorted by due date with the soonest (and anything overdue) first. Your
undated, whenever-you-get-to-them items stay up top; everything with a
clock ticking clusters together near the bottom where its date rows line
up. This happens on every list, whether or not you sort checked items to
the bottom.
...
Deadlines travel with your lists across devices, and on the file and
cloud backends they're written into the markdown in a plain, readable
form — `*(due 2026-07-20, every 2 weeks)*` — so the date is visible even
if you open the file in another editor.
