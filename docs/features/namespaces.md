...
A **namespace** is a named, self-contained group of checklists. Everything you
keep in one namespace — its lists, their items, their archive — stays separate
from every other namespace, so you can run a personal set of lists and a shared
family set side by side without one bleeding into the other. There is always a
