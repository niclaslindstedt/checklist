...
A couple of things worth knowing. iCloud syncs your **lists**; your appearance settings and your set of [namespaces](feature:namespaces) stay on each device rather than travelling with it, so if you keep several namespaces you'll set them up per device. And iCloud is Apple's key-value store rather than a folder of files, so unlike [Dropbox, or a local folder](feature:cloud-sync) there are no per-list markdown files to open — the trade for needing no account at all.
