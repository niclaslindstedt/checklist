...
// The native wrapper (`native/`) embeds this bundle and serves it from a
// loopback origin inside a WebView. That build drops the PWA layer: the
// assets already ship inside the app binary, so a service worker would add
// a second, staler cache in front of them, and the "new version available"
// prompt is meaningless when updates arrive through the App Store. Guards
