---
name: tune-pwa-icons
description: "Use when the home-screen / launcher / browser-tab icon looks wrong on a real device — too small, off-center, transparent, clipped by iOS rounding, or eaten by an Android mask — or when restyling checklist's PWA artwork. Walks an edit / regenerate / inspect loop that uses the Read tool to look at the PNGs after every change, scored against Apple Human Interface Guidelines and the W3C maskable-icon spec. checklist derives every PWA icon from a single source SVG via `@vite-pwa/assets-generator` (`make icons`); the manifest `icons` array lives inline in `vite.config.ts`. Manual playbook — not part of the `maintenance` umbrella."
---

# Tuning the PWA icon set

checklist derives its whole PWA icon set from **one source SVG** with
...
Running `make icons` (→ `npm run icons` → `pwa-assets-generator`) reads
...
The generated PNGs **and** `favicon.ico` are committed to the repo. The
`icons-check` CI job (in `.github/workflows/ci.yml`, via the
`icons-check` Make target) regenerates into a temp dir and fails the
build if any committed file differs from what `public/favicon.svg`
produces — so an edited SVG that ships without rerunning `make icons`
is caught. **Always `make icons` and commit the result in the same
change as a `favicon.svg` edit.**
...
edge-to-edge `apple` / `any` icons, which want the glyph bigger.

## Common pitfalls

In roughly descending order of likelihood:

1. **Editing `favicon.svg` without rerunning `make icons`.** The
   committed PNGs go stale and the `icons-check` CI job fails. The two
   move together — regenerate and commit in the same change.
2. **Hand-editing a generated PNG.** Pointless: the next `make icons`
   overwrites it, and `icons-check` flags the divergence meanwhile.
   Change the source SVG or the config instead.
...
5. **Editing the manifest `icons` array but not the generator (or vice
...
6. **Forgetting the `apple-touch-icon` `<link>` in `index.html`.**
   ...
   hand.
7. **Not rebuilding before judging.** `make icons` produces the PNGs,
...
- [ ] `make icons-check` passes (no drift between `favicon.svg` and the
      committed PNGs).
...
2. `make icons-check` passes (committed icons match `favicon.svg`).
...
8. Whether an icon/manifest change needs a changelog entry depends on
   user visibility — restyling the home-screen icon is user-visible; a
   sub-pixel nudge usually is not. Run the `write-changeset` skill to
   decide between a `.changes/unreleased/` fragment and `no-changelog`.
...
1. If a new generator quirk bit the run (a preset default that needed
   overriding, an SVG feature the rasteriser dropped), add it to
