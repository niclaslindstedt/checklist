...
1. **Resolve the baseline — the latest released commit.** Releases are
   ...
   unreleased in `.changes/unreleased/`.
...
```sh
TS=$(date +%s)
SLUG=short-kebab-case-summary    # e.g. share-link-compression, dropbox-app-folder
...
Voice: second-person, present tense, user-centric ("Checklists can now be
shared as a link"), not implementation-centric ("Wired the share encoder
into the toolbar"). The fragment is read by someone who has never read the
source tree.
...
3. **The fragment reads correctly to a stranger.** If it's full of
   internal terminology (`StorageBackend`, file paths), rewrite it in the
...
- **Writing `type: Fixed` for a fix on an unreleased feature.** The notes
  would say "Fixed: the new picker no longer flickers" — but the picker
  has never been in production, so no user saw the flicker. Fold it into
  the picker's `Added` fragment instead, or label `no-changelog`.
