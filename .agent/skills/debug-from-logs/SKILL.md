...
git grep -n "failed to decode share payload"
...
- "`src/storage/local/index.ts:142` reads a value the
  `StorageBackend` interface declares as `string`, but a pre-v2
  payload stored a raw object, so `JSON.parse` throws on first
  load."
- "`src/share/index.ts:88` decodes the URL fragment with
  `atob`, but the encoder switched to a URL-safe base64 alphabet,
  so any share link containing `-` or `_` fails to round-trip."
...
pure (no DOM, no I/O, no imports from `ui/`, `storage/`, `window`,
`document`, or `fetch`), so a domain bug is reproducible in a plain
node test with no environment setup. A bug that only manifests in
the browser lives in `src/ui/`, `src/app/`, `src/storage/`, or
`src/pwa/`.
...
`.test.ts` suffix (e.g. a bug in `src/share/index.ts` gets
`tests/share/index.test.ts`). The test should:

1. Exercise the same path the trace describes — call the domain or
   share/storage function directly with the input that reproduces
   the failure. Mock storage at the `StorageBackend` interface;
...
3. Pick the environment deliberately. Domain and share/storage
   tests run in plain node. A UI test that needs the DOM opts in
   with a `// @vitest-environment jsdom` docblock at the top of the
   file.
...
glitch, a focus trap, an install-prompt or service-worker issue —
run the dev server and reproduce it by hand:
...
- Add a Vitest regression test for the class of bug, not just the
  one input, when the surface has obvious neighbours (every share
  payload shape, every storage backend, etc.).
...
- `make test` runs the full suite (`vitest run`). Scope to one file
  with `npx vitest run tests/share/index.test.ts`, or iterate with
  `npx vitest tests/share/index.test.ts` (watch mode).
- Tests live in `tests/`, mirroring the `src/` tree, with a
  `.test.ts` suffix.
- Domain and share/storage tests run in plain node. A UI test that
  needs the DOM opts into jsdom with a `// @vitest-environment jsdom`
  docblock at the top of the file.
- Mock storage at the `StorageBackend` interface (`src/storage/types.ts`)
  — never reach into `localStorage` directly from a test. A
  reproduction that depends on stored state should set that state up
  through the interface inside the test.
...
- `src/domain/` is pure by contract. A stack trace whose frames are
  all inside `src/domain/` is reproducible in node with zero
  environment setup — write the failing test first and you've
  likely already isolated it.
...
4. If the bug was hard to reach from a plain function call (it
   needed jsdom setup or a storage mock you had to hand-roll), add a
   small helper under `tests/` so the next regression on the same
   surface starts from a known state.
