...
- **Past-state framing**: "Previously this did X", "used to
  re-scan the whole list", "the legacy behaviour", "before this
  fix", "the original implementation", "the prior version".
- **Perf comparison against the deleted approach**: "Trades two
  O(N log N) sorts for one", "without this the encoder ran
...
- **Wiring narratives**: "Wired the new toggle through
  ...
  out of the local backend so every `StorageBackend` can reuse
...
- **WHY a non-obvious choice was made**: "Encoded in the URL
  fragment, never the query string — fragments aren't sent to the
  server", "null is the explicit-clear signal, undefined is
  `never set`", "the sort must be stable because ties depend on
  insertion order".
- **Hidden constraints and invariants**: "This module is in
  `domain/` — it must stay pure: no DOM, no `fetch`, no imports
  from `ui/` or `storage/`", "every `StorageBackend` method must
  resolve even when the remote is offline — callers don't
  re-check", "this regex assumes `^…$` anchors elsewhere".
- **Magic-number rationale**: "Cap the fragment at ~2 KB —
  browsers truncate long URLs and the share link silently breaks
  past that".
- **Anti-pattern guards** ("don't try X — it doesn't work
  ...
  experiment. Examples: "Do not switch the service worker off
  `autoUpdate` without wiring a prompt flow — with manual update
  the `waiting` worker is never activated", "Do not refactor this
  to `Array.from(entries)` — the iterator is consumed by the
...
- **Cross-file invariants**: "Mirrors the encode order in
  `share/encode.ts` — keep them in sync or old links won't
  decode".
...
- ❌ "Decode the fragment field-by-field. Previously we
  `JSON.parse`'d the whole blob and re-scanned it per item, which
  was O(N²) on big lists; this walks once so `decodeChecklist`
  stays O(N)."
- ✅ "Decode the fragment field-by-field in one walk so
  `decodeChecklist` stays O(N) on large lists."

- ❌ "Cap the iteration to prevent runaway in pathological
  inputs. This was added after a list with thousands of items
  hung the encoder; keeps the worst case bounded regardless of
  input size."
...
One module at a time. `src/share/encode.ts` is a scope.
`src/share/` is not. The diff for a single module is reviewable;
...
- **Deleting an anti-pattern guard because it reads like
  history.** "Do not switch the service worker off `autoUpdate`
  — the `waiting` worker is then never activated" sounds like a
...
- **Treating commit-message-style imperative comments as
  history.** "// Normalize the slug before encoding so old and
  new links decode identically" is fine — it states a current
  rule with WHY. The "before encoding" is sequencing, not
  history.
...
- **"Hot path: this runs on every keystroke. Two perf moves keep
  ...
  is history.
- **"Used by the local, Drive, and Dropbox backends so they all
  …"** — caller-genealogy. Callers change; the comment doesn't.
  Delete or rewrite to state the _contract_ the callers depend
