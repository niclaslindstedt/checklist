// On-demand loading for the non-default webfont families. Cloned from
// the budget project's `utils/fonts.ts`.
//
// The default family (JetBrains Mono, the `mono` id) is imported
// statically in `src/app/main.tsx`, so it's part of the main bundle and
// precached for offline first paint. The three non-default families load
// lazily — when the user selects one (`useTheme`) or when the Appearance
// font picker mounts to render its previews.
//
// Only the latin + latin-ext subsets ship: the app's UI text lives
// entirely within them, so fontsource's bare `400.css` / `700.css`
// entrypoints — which also pull cyrillic / greek / vietnamese — would be
// pure waste. OpenDyslexic is latin-only upstream, so it has no
// latin-ext import.
//
...
/**
 * Kick off every non-default family so the font-picker previews render
 * in their real face. Fire-and-forget — previews swap in as each lands.
 */
