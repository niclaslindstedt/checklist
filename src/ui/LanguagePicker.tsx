// Two-flag language picker used by the General settings tab. Renders the
// UK flag (English) and the Swedish flag side by side; the active language
// gets full opacity and an accent border, the inactive one dims to make the
// contrast obvious at a glance. Ported from budget's `LanguagePicker`.
//
...
// systems without colour-emoji fonts (notably Windows), and the inline SVGs
...
export function LanguagePicker({
  ...
  const t = useT();
  // Clicking the already-active flag is a no-op: re-selecting the current
  // language would still dispatch the runtime switch event (and so fire the
  // `polyglot` achievement) without a real change.
