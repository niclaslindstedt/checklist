// Keep-a-Changelog parser. Ported from budget's build-time changelog
// plugin, but run at module load over the raw `CHANGELOG.md` text Vite
// inlines (`?raw`) — the changelog is small and only the menu's
// "What's new" modal reads it, so a build-time codegen step would be
// more machinery than the feature warrants. Pure and DOM-free so it is
// trivially testable.
...
export interface ChangelogRelease {
  // "Unreleased" or a semver like "0.1.0".
  version: string;
