...
// Slide-down pill that surfaces the pull-to-refresh gesture, ported from
// budget. Pinned to the top edge of the visible viewport (below the iOS
// safe-area inset) and translated by `pullDistance` so it appears to
// emerge from behind the header as the user pulls. Shares the same
// border / surface tokens as `UpdateToast` so the two affordances feel
...
export function PullToRefreshIndicator({ state, pullDistance }: Props) {
  ...
  // The indicator slides from above the viewport into place. The -44px
  // floor matches the pill's approximate rendered height so it sits flush
  // above the page until pulled.
  ...
  // during the live drag, tracking must be 1:1 so the pull feels attached
  // to the finger.
