...
// Adapted from budget's `AchievementsModal` to the checklist's flatter
// `Modal` (header / scroll body, no Modal.Header subcomponents) and inline
// glyphs. Reads the unlocked map straight from the synced settings passed in
// from App, so it stays correct under any storage backend.
...
export function AchievementsModal({ open, onClose, unlocked }: Props) {
  ...
  const knownIds = unlocked
    ? Object.keys(unlocked).filter((id) =>
