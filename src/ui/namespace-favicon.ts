...
// wordmark slot and as the browser-tab favicon. Without a glyph, the
// bundled `public/favicon.svg` is used unchanged.
 ...
 * chosen, otherwise the bundled favicon. A namespace with only a colour
 * (no glyph) keeps the bundled mark — the favicon is replaced only when a
 * glyph is picked.
 */
