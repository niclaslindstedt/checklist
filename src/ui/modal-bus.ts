...
// Modal command-bus, sized down from the budget project's. It decouples
// *who opens a modal* (any button in the tree calls `dispatch`) from *who
// owns its open/close state* (a host component reads `useModalState`).
// Adding a modal is then a new host file + one arm on `ModalCommand` — no
// new `useState` in the app root and no opener prop threaded through
// `SideMenu`, the hub files that otherwise collide on every feature branch.
//
// Only one bus modal is open at a time (opening another replaces it),
 ...
 * closed, which also carries the command's payload, e.g. the settings tab)
 * plus a `close` to dismiss it.
 */
