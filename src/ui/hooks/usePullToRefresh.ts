...
// Touch-driven pull-to-refresh, ported from budget. Listens at the
// document level for a downward drag that starts while the scroll region
// is at the top, applies rubber-band damping, and fires `onRefresh` once
// the user crosses `TRIGGER_DISTANCE` and lets go.
//
// Deliberately touch-only — pull-to-refresh is a mobile gesture. Pointer
// events would also bake in "mouse drag refreshes the page" behaviour,
// which feels wrong with a trackpad's two-finger scroll.
//
// Scroll gate: checklist pins the shell to the viewport (`html`/`body`
// are `overflow: hidden`, see styles/theme.css) and scrolls the item
// list inside its own region, so `window.scrollY` is always 0. Instead
// we walk up from the touch target and require every scrollable ancestor
// to be at its top — a downward drag while the list is scrolled mid-way
// should scroll the list up, not arm a refresh.
//
// Modal gate: while any `[aria-modal="true"]` element is mounted (the
// settings dialog sets it), the gesture is suppressed so a downward drag
// inside a modal can't accidentally trigger a refresh of the chrome
// behind it.
...
// reachable in one thumb travel.
const TRIGGER_DISTANCE = 70;
// Max damped distance the indicator will travel. Past this point further
// pulling does nothing visually — it keeps the indicator from running off
// the band and signals "yes, it's armed; you can let go".
const MAX_PULL = 110;
...
// indicator moves half as far as the finger, matching the iOS-native
// pull-to-refresh feel closely enough.
...
type Result = {
  ...
  // Current damped pull distance in px (0..MAX_PULL). The indicator uses
  // this to translate / fade itself in as the user pulls.
  pullDistance: number;
};
...
// True only if every scrollable ancestor of `target` is scrolled to its
// top. A non-scrolled list lets the pull arm; a mid-scrolled one hands
...
export function usePullToRefresh(
  ...
  // `onRefreshRef` and only call `setState` / `setPullDistance` when the
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      // Also covers the "user pulled down a bit, then up past start" hand
      // wobble.
      if (delta <= 0) {
        resetIdle();
        return;
      ...
      // Android's URL-bar pull-to-refresh) while we own the gesture. Only
      // call once the gesture is armed — leaving every touchmove passive
      // when we're not pulling keeps page scroll smooth.
    ...
    const onTouchEnd = () => {
      ...
      if (distance >= TRIGGER_DISTANCE) {
        ...
        // Pin the indicator at the trigger position while the refresh is
        // in flight so it doesn't snap back before the user sees the
        // spinner.
