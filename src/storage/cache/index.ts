...
// cache (this device's localStorage) so the document can be read — and
// edited — while the network is unreachable (airplane mode, a dead tunnel,
// a captive portal). It sits between a cloud adapter and the encryption
// wrapper, so the cached bytes are exactly what the cloud holds: an
// AES-GCM envelope when encryption is on (the cache never sees plaintext
// then), or the canonical document JSON when it isn't.
//
// Why here and not inside each cloud adapter: the mirror logic —
// write-through on every successful load / save, fall back to the cache on
// a *network* failure, and leave the typed errors (auth / conflict /
// rate-limit) alone so their upstream handling still fires — is identical
// for Dropbox and Google Drive. So it lives once in this wrapper the way
// `withEncryption` wraps the byte boundary once.
//
...
// passphrase against — which is what lets unlocking work offline: the
// cached envelope is enough to check the passphrase and render the lists
...
export type LocalCacheOptions = {
  ...
  /** Storage key, namespaced by backend id + namespace by the caller. */
  key: string;
};

// A failure means "serve the cache" only when it's a raw network error,
// never one of the adapter's typed signals: a `ConflictError` /
// `AuthError` / `RateLimitError` each has dedicated handling upstream, and
// quietly returning a stale cached read instead would mask it.
...
export function localCacheKey(backendId: string, namespace: string): string {
  return `checklist:cache:${backendId}:${namespace}`;
}
  ...
  return {
    ...
    async load(): Promise<StoredSnapshot | null> {
      try {
        ...
        // the caller decide — `useStorageBackend.unlock` maps the latter to
        // a distinct "you're offline" message.
        throw err;
      }
    },
    ...
    async save(text: string, baseRevision?: string): Promise<StoredSnapshot> {
      try {
        ...
        if (isOfflineError(err)) {
          ...
          // returns (see the `online` listener in `use-checklist-sync`).
