...
// `checklist/`, the picked local directory). It sits beside `settings.json`
// and the per-namespace folders, so the namespace list travels with the
...
// of any one checklist document. It stays plaintext JSON even when the
// documents are encrypted — a namespace's name/icon isn't secret, and the
// list has to be readable before the unlock gate can render.
//
...
// `settings.json` and the namespace folders (`default/`, `family/`, …),
// never inside one.
 ...
 * folder — one constructed with an empty namespace, so its relative paths
 * resolve directly under the app-folder root rather than inside a namespace
 * folder. Reads / writes the single `namespaces.json` file there.
 */
