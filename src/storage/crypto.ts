...
// `crypto.ts`.
//
// Defaults follow OWASP 2023 password-storage guidance: PBKDF2-SHA256
// at 600k iterations, AES-GCM with a 256-bit key, a fresh random salt
// per envelope, and a fresh 12-byte IV per encryption. The KDF
...
// Coarse phases an encrypt/decrypt passes through, fired so the settings UI
// can flash a one-line "this is what's happening" status while the
// (deliberately slow) 600k-iteration key derivation runs. Optional and
// side-effect-only — the crypto result is unchanged whether or not a callback
// is supplied.
export type CryptoProgressStep = "derivingKey" | "encrypting" | "decrypting";
export type CryptoProgress = (step: CryptoProgressStep) => void;

// Parse without throwing — returns `undefined` for malformed input so
// the envelope sniffers below can treat "not JSON" the same as "not an
// envelope". Inlined here because the checklist has no shared JSON util.
