// Forward-only migration chain for the persisted document, adapted from
// the budget project's `data/migrations` framework. The stored JSON
// carries a top-level numeric `version`; each entry in the table
// migrates from version `N` to `N+1`. Once a migration ships it must
// never be removed or rewritten — documents in the wild (and exports)
// still depend on it to upgrade cleanly.
//
...
// `src/domain/types.ts` in the same commit.
//
...
const migrations: MigrationTable = {
  // v0 → v1: the bootstrap step. Documents written before versioning
  // existed (a bare `{ templates, checklists }`) read as version 0; this
  // normalises them by guaranteeing both top-level arrays are present so
  // a partially-written legacy file still upgrades cleanly.
  0: (doc) => ({
    ...doc,
    version: 1,
...
// Throws when the document was written by a newer build than this one,
// or when a step in the chain is missing.
...
// present-but-non-numeric `version` is also coerced to 0 so a corrupt
// header re-runs the chain from the start rather than throwing.
