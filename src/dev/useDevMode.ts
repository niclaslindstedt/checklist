...
//
// Both are stored outside the appearance `Settings` so they don't travel
// with an export — they're device-local diagnostics. Turning dev mode
...
// immediately, not on the next reload. (The browser only fires the
// `storage` event in *other* tabs.) Cloned from the budget project's
// `useDevMode`, minus the preview-build gating — the checklist exposes
// developer mode in every build.
...
// one window propagates to every open tab.
