...
// flipped from false to true on this (prev → next) transition AND that
// aren't already in `alreadyUnlocked`. Manual triggers are skipped — they
// fire through the bus, not the state watcher. Adapted from budget's
// `deriveUnlocks`.
//
...
// the state island they didn't touch (a document edit replaces only
// `snapshot`, an appearance change only `settings`), so a document-only edit
// can't flip a settings-only predicate. Cheap pre-check that avoids the
// full-snapshot walks several predicates do.
