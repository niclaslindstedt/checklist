...
//
// English is bundled (the default + fallback + the `Catalog` /
// `MessageKey` type source); every other language is code-split and
// loaded on demand via `ensureCatalog`, so a language the user never
// selects costs nothing at first paint. Lookups stay synchronous —
// `t()` falls back to English for any key whose catalog isn't resident
// yet, and `LanguageRoot` loads the active catalog before applying it so
// that fallback is a safety net rather than a visible state.
...
// resolves `t("a.b.c")` as a single `Map.get` instead of walking the
// nested object on every call.
...
// users, and a guaranteed synchronous answer for any key in any language
// (a not-yet-loaded catalog falls through to this map). It is also the
// source of the compile-time `Catalog` / `MessageKey` types.
const flatEn = flatten(en);

// Every *other* language is code-split: its catalog loads on demand via
// `ensureCatalog`, keyed by the loader registry below, and is flattened
// into this map once it arrives. Adding a language is: extend `Lang`,
// add a `locales/<code>/` dir, and add one line to each registry below.
...
// De-dupe concurrent loads of the same language (StrictMode's double
// effect, a render firing before the first load resolves) so a catalog
// is fetched and flattened at most once.
...
export function useT(): TFunction {
  ...
  return useCallback<TFunction>(
    (key, params) => {
      ...
      // loaded yet. `LanguageRoot` gates the first paint so this only
      // ever surfaces briefly, if at all, during a runtime switch.
