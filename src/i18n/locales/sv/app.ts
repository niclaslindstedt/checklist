import type { AppCatalog } from "../en/app";

const app: AppCatalog = {
  title: "checklist",
  empty: "Inget här än — lägg till din första rad nedan.",
  renameChecklist: "Byt namn på checklista",
  changeListIcon: "Ändra listans ikon och färg",
  listColorLabel: "Färg",
  listGlyphLabel: "Ikon",
  listGlyphNone: "Checklista (standard)",
  addItem: "Lägg till rad",
  addItemPlaceholder: "Lägg till rad…",
  suggestions: "Förslag från arkivet",
  check: "Bocka av rad",
  uncheck: "Avmarkera rad",
  checkAll: "Bocka av alla",
  uncheckAll: "Avmarkera alla",
  archive: "Arkivera",
  delete: "Ta bort",
  promoteToCategory: "Gör till kategori",
  demoteFromCategory: "Ta bort kategori",
  setTiming: "Ange tidsplan",
  timing: {
    title: "Tidsplan",
    notBefore: "Tidigast",
    dueDate: "Förfallodatum",
    pickDate: "Välj ett datum",
    repeat: "Upprepa",
    noRepeat: "Upprepas inte",
    every: "Var",
    interval: "Upprepningsintervall",
    unitDay: "dag",
    unitWeek: "vecka",
    unitMonth: "månad",
    unitYear: "år",
    at: "Klockan",
    hour: "Timme",
    minute: "Minut",
    refreshHint:
      "Utan förfallodatum kommer raden tillbaka omarkerad högst upp i listan varje gång.",
    clear: "Ta bort tidsplan",
    overdue: "Försenad",
    repeats: "Upprepas {summary}",
    everyAt: "{cadence} kl. {time}",
    everyDayOne: "varje dag",
    everyDayOther: "var {n}:e dag",
    everyWeekOne: "varje vecka",
    everyWeekOther: "var {n}:e vecka",
    everyMonthOne: "varje månad",
    everyMonthOther: "var {n}:e månad",
    everyYearOne: "varje år",
    everyYearOther: "var {n}:e år",
  },
  resetSchedule: {
    title: "Återställningsschema",
    hint: "Avmarkera alla rader i ”{name}” enligt ett schema.",
    every: "Nollställ var",
    interval: "Intervall",
    unitDay: "dag",
    unitWeek: "vecka",
    unitMonth: "månad",
    unitDayOfWeek: "veckodag",
    days: "Dagar",
    pickDays: "Välj dagar",
    everyDay: "Varje dag",
    at: "Klockan",
    hour: "Timme",
    minute: "Minut",
    popUp: "Visa efter nollställning",
    popUpHint:
      "Visa den nyss nollställda listan i en ruta nästa gång du öppnar appen.",
    next: "Nästa nollställning: {when}",
    never: "Välj minst en dag.",
    remove: "Ta bort schemat",
    popupTitle: "Nystart",
    popupHint: "Den här listan nollställdes nyss.",
    popupEmpty: "Inget på den här listan än.",
    openList: "Öppna listan",
  },
  editItem: "Redigera rad",
  editNote: "Redigera anteckning",
  editTitlePlaceholder: "Radtext…",
  notePlaceholder: "Lägg till en anteckning… (markdown stöds)",
  addNote: "Lägg till en anteckning",
  addSubitem: "Lägg till underpunkt",
  addToCategory: "Lägg till punkt i den här kategorin",
  showNote: "Visa anteckning",
  hideNote: "Dölj anteckning",
  showSubitems: "Visa underpunkter",
  hideSubitems: "Dölj underpunkter",
  moreActions: "Fler åtgärder",
  archiveFinished: "Arkivera avklarade",
  deleteFinished: "Ta bort avklarade",
  dragToReorder: "Dra för att ändra ordning",
  openSettings: "Öppna inställningar",
  templateItem: "Mallpost (kan inte bockas av)",
  templateBannerHint: "Mall — poster bockas inte av här",
  templateUse: "Ny lista från denna",
  saveAsTemplate: "Spara som mall",
  deleteTemplate: "Ta bort mallen",
  copyChecklist: "Kopiera checklista som markdown",
  copied: "Kopierad",
  copyFailed: "Kunde inte kopiera till urklipp",
  copyScope: "Kopiera hela listan eller en kategori",
  copyAll: "Allt",
  itemCount: "{checked} av {total} poster bockade",
  itemCountActions: "Bocka av eller avmarkera alla poster",
};

export default app;
