...
// `useChecklist` hook drive it unchanged.
//
// Why key-value and not iCloud Documents: a checklist document is small
// (well under the 1 MB per-key / ~64 KB-per-value envelope KVS allows for a
// JSON blob of this size) and the store syncs itself across the user's
// devices with no OAuth, no tokens, and no network code of our own. That
// keeps the adapter to the same getItem/setItem shape as the local one.
//
...
// as AsyncStorageAdapter, and `useChecklistSync` already tolerates that.
//
