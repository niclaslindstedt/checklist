...
// shared serialize / parse / migration pipeline and the `useChecklist` hook
...
// the `loadSync` capability. `useChecklistSync` already tolerates that — it
// seeds an empty document on first paint and fills it from `load()` in its
// mount effect — so there is no behavioural gap, just no pre-paint data.
