// THE AUTH-SESSION BRIDGE: how the page signs in to Dropbox from inside the app.
//
// The page is served in a WebView from a loopback origin, and its OAuth
// redirect flow cannot finish there. Dropbox refuses to show consent in an
// embedded WebView, so `App.tsx` sends every off-origin page to the system
// browser — and Dropbox then redirects THAT browser to `http://localhost:…`:
// an origin it does not have registered, in a browser that does not hold the
// PKCE verifier the page stashed in its own `sessionStorage`. Connecting could
// never complete.
//
...
// sheet over the app that closes the moment the provider redirects to a URI the
// app claims, and hands that URI back. This wrapper offers one to the page as a
// CAPABILITY at `window.__ossAuthSession` — the seam the page's
// `getAuthSessionHost` looks for (`src/storage/auth-session.ts`, the same names
// as oss-framework's). A browser has no such host and keeps its redirect flow;
// the desktop shell has none and keeps its loopback one.
//
...
// exchange all stay in the page, which is the only place the tokens exist.
//
// This file exports STRINGS for the page (dependency-free, ES5-ish — nothing
// in them is transpiled) plus the pure narrowing helpers, and it is exercised
// from the root test suite (`tests/native/auth-session-bridge.test.ts`), so it
// imports nothing that reaches `expo` or `react-native`.

/** The message the page posts to ask for a session. Namespaced so it is never
 *  confused with a theme report or a `window.__native` bridge call. */
export const AUTH_SESSION_REQUEST_TYPE =
  "checklist-native/auth-session-request";

/** Where the provider installs itself — the PAGE's name for it
 *  (`AUTH_SESSION_HOST_PROPERTY`), so a rename here is not an error but a
 *  Dropbox button that goes back to opening Safari. The tests pin the two. */
...
/** The resolver the settling script calls. Wrapper-private; the page never
 *  reads it. */
...
/**
 * The redirect URI the wrapper catches: `<scheme>://oauth`, where `<scheme>` is
 * `app.config.js`'s `scheme` — the bundle id, so `se.agilator.checklist://oauth`
...
/** The answer. `value` is the URL the provider redirected to, or null when the
 *  reader closed the sheet. A failure crosses as data, because the channel
 *  carries strings; the page-side script turns it back into a thrown `Error`. */
export type AuthSessionResult =
  { ok: true; value: string | null } | { ok: false; error: string };

/** A JavaScript string literal holding `text`, safe to splice into a script.
 *  `JSON.stringify` does the quoting; U+2028 and U+2029 are the two characters
 *  it leaves alone that an older parser reads as line breaks. */
